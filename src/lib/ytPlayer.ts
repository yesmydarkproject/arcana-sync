...
/* https://github.com/feross/yt-player/tree/a244829e8aee2eb93b92a35a8035e8efd71cecfd
 * Typings & other changes: YesMyDark Project contributors
 */
...
type YouTubeStateKey = keyof typeof YOUTUBE_STATES;
...
type ApiCallback = (err: Error | null, api?: any) => void;
const loadIframeAPICallbacks: ApiCallback[] = [];
...
export default class YouTubePlayer extends EventEmitter {
  ...
  private _opts: YouTubePlayerOptions | null;
  videoId: string | null = null;
  destroyed = false;
  private _api: any = null;
  private _autoplay = false; // autoplay the first video?
  private _player: any = null;
  private _ready = false; // is player ready?
  private _queue: [string, unknown[]][] | null = [];
  private _interval: number | null = null;
  private _start: number | undefined = undefined;

  constructor(element: string | HTMLElement, opts: YouTubePlayerOptions) {
    super();
    ...
    this._opts = Object.assign<YouTubePlayerOptions, YouTubePlayerOptions>(
      {
        width: 640,
        height: 360,
        autoplay: false,
  ...
  getState() {
    return (
      (this._ready && YOUTUBE_STATES[this._player.getPlayerState() as YouTubeStateKey]) ||
      'unstarted'
    );
  }
  ...
  _queueCommand(command: string, ...args: unknown[]) {
    if (this.destroyed) return;
  ...
  _flushQueue() {
    ...
    while (this._queue.length) {
      const command = this._queue.shift() as (typeof this._queue)[number];
  ...
  _loadIframeAPI(cb: ApiCallback) {
    ...
    if (!isLoading) {
      loadScript(YOUTUBE_IFRAME_API_SRC).catch((err: Error) => {
  ...
  _createPlayer(videoId: string) {
    if (this.destroyed) return;

    const opts = this._opts;
    if (!opts) throw new Error('opts is null');
    ...
    this._player = new this._api.Player(this._id, {
      ...
      events: {
        ...
        onStateChange: (data: any) => this._onStateChange(data),
        onPlaybackQualityChange: (data: any) => this._onPlaybackQualityChange(data),
        onPlaybackRateChange: (data: any) => this._onPlaybackRateChange(data),
        onError: (data: any) => this._onError(data)
      }
    });
  }
  ...
  _onReady() {
    ...
    if (!this.videoId) throw new Error('this.videoId is null');
   ...
   */
  _onStateChange(data: { data: YouTubeStateKey }) {
    if (this.destroyed) return;
   ...
   */
  _onPlaybackQualityChange(data: { data: string }) {
    if (this.destroyed) return;
   ...
   */
  _onPlaybackRateChange(data: { data: number }) {
    if (this.destroyed) return;
